...
import { CardBodyStylesType } from "../theme";
...
export const CardBody = React.forwardRef<HTMLDivElement, CardBodyProps>(
  ({ className, children, ...rest }, ref) => {
    ...
    const {
      defaultProps,
      styles,
    } = cardBody as Required<CardBodyStylesType>;

    // 2. set default props
    className = className ?? defaultProps.className;

    // 3. set styles
    const cardBodyClasses = twMerge(classnames(objectsToString(styles.base)), className);
